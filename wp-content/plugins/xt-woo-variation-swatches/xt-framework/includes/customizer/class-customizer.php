    ...
    class XT_Framework_Customizer {
        ...
        public function customizer_tab_section() {
            ...
            if(!$this->is_module && $this->core->has_modules(true)) {
	            ...
	            foreach($this->core->modules()->all(true) as $module) {
                    ...
                    if(!empty($this->panels)) {
                        echo '<h4 class="xtfw-modules-title">' . $module->menu_name() . ' <span class="xtfw-badge xtfw-badge-blue">'.__('Shared Module', 'xt-framework').'</span></h4>';
                    }
	    ...
	    public function render_customizer_tab_sections(&$customizer = null, $panel = null) {
		    ...
		    foreach($customizer->sections as $section) {
			    ...
			    echo '<li><a title="'.esc_html__('Open in Customizer', 'xt-framework').'" href="'.$customizer->customizer_link(null, $section['id']).'">';
			    if(!empty($section['icon'])) {
                    echo '<span class="dashicons ' . $section['icon'] . '">';
                }
			    echo '</span> '.$section['title'].'</a></li>';
		    }
